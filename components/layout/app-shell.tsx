...
})
{
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[var(--surface-container)] text-[var(--on-surface)]">
      {showHeader ? (
        <header className="app-navbar sticky top-0 z-40 border-b border-[color:var(--header-divider)] bg-[var(--header-surface)] shadow-[0_4px_18px_rgba(15,23,42,0.08)] backdrop-blur">
                    ...
                    {navItems.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          prefetch
                          href={item.href}
                          className={`app-nav-link inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-[999px] px-2.5 py-1.5 text-[12px] font-semibold leading-4 transition-colors sm:gap-2 sm:px-3 sm:py-2 ${
